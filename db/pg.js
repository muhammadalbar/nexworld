...
async function registerUser(uid, email, password, role, props = {}) {
  try {
    let { rows } = await db.query(
      "insert into users (uid, email, password, role, props, register_date) values ($1, $2, $3, $4, $5, $6)",
      [
        uid,
        email,
        password,
...
async function updateUser(email, props) {
  try {
    let { rows } = await db.query(
      "update users set props = $2 where email = $1",
      [email, props]
    );
...
async function reRegisterUser(email, password, props) {
  try {
    let { rows } = await db.query(
      "update users set (password, props) = ($2, $3) where email = $1",
...
async function getAnnotations() {
  try {
    let { rows } = await db.query(
      "SELECT * FROM annotations order by number ASC"
    );
    return rows;
  } catch (err) {
    return { error: true, message: err.toString() };
...
async function getAnnotation(uid) {
  try {
    let { rows } = await db.query("SELECT * FROM annotations where uid = $1", [
      uid,
    ]);
    return rows;
  } catch (err) {
...
async function getAnnotationsSpecificByNumber(boothnumber, annotationnumber) {
  try {
    let { rows } = await db.query(
      "SELECT * from annotations where boothid = (select uid from booths where number = $1) and content->>'number' = $2",
      [boothnumber, annotationnumber]
    );
...
async function addAnnotation(uid, boothid, aname, acontent) {
  try {
    let { rows } = await db.query(
      "INSERT into annotations(uid, boothid, name, content) values ($1, $2, $3, $4)",
      [uid, boothid, aname, acontent]
    );
...
async function deleteAnnotation(uid) {
  try {
    let { rows } = await db.query("DELETE from annotations WHERE uid = $1", [
      uid,
    ]);
...
async function editAnnotation(uid, aname, acontent) {
  try {
    let { rows } = await db.query(
      "UPDATE annotations SET (name, content) = ($2, $3) where uid = $1",
      [uid, aname, acontent]
    );
...
async function getBriefcase(email) {
  try {
    let { rows } = await db.query(
      "SELECT props -> 'briefcase' AS briefcase from users where email = $1",
      [email]
    );
...
async function addBriefcase(email, briefcase) {
  try {
    ...
    let findBriefcase = rows[0].briefcase.find(
      (bc) => bc.file == briefcase.file
    );
    if (findBriefcase) {
      return { error: true, message: "Briefcase already exist!" };
    } else {
      let response = await db.query(
        `UPDATE users SET props=jsonb_set(props, '{briefcase}', (props->'briefcase') || $2) where email = $1`,
        [email, parseBriefcase]
      );
