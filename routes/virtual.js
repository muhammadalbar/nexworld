...
router.post("/briefcase", authMw.authToken(), async (req, res) => {
  try {
    let response = await pgdb.addBriefcase(req.body.email, {
      file: req.body.briefcase,
      name: req.body.name,
    });
...
router.get(
  "/getAnnotation/:booth/:annotation",
  authMw.authToken(),
  async (req, res) => {
    try {
      let response = await pgdb.getAnnotationsSpecificByNumber(
        req.params.booth,
